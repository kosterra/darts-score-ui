    ...
    if (active && payload && payload.length) {
      return (
        <div className="d-flex flex-column align-items-center bg-tertiary">
          <span className="bg-primary text-shade100 text-center fs-8 p-2 w-100">{`${payload[0].payload.section}`}</span>
          <div className="d-flex flex-column align-items-center p-2">
            <span className="text-shade100 fs-8 pb-1">{`${payload[0].payload.rate + '%'}`}</span>
            <span className="text-shade100 fs-9">{`${'(' + payload[0].payload.hit + '/' + payload[0].payload.total + ')'}`}</span>
          </div>
        </div>
      );
    }
