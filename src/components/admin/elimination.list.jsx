...
const EliminationList = (props) => {
    ...
    const onEliminationGameDelete = async (game) => {
        if (await EliminationService.deleteEliminationGame(game.id)) {
            toast.current.show(
                {
                    severity: 'success',
                    summary: 'Elimination Game deleted',
            ...
            toast.current.show(
                {
                    severity: 'error',
                    summary: 'Elimination Game deleted',
                    detail: 'Error on deleting Elimination game: ' + game.id + '. Please try again or contact your system administrator',
                    life: 3000
                }
            );
        }
    ...
    const loadEliminationGames = async () => {
        let data;
        if (statusFilterValue === 1) {
            data = await EliminationService.loadRunningEliminationGames();
        } else if (statusFilterValue === 2) {
            data = await EliminationService.loadFinishedEliminationGames();
        } else {
            data = await EliminationService.loadAllEliminationGames();
        }
        setEliminationGames(data);
    }
                    ...
                    <div className="container-fluid overflow-hidden">
                        <div className="row gy-4">
                            ...
                            <div className="col-12 col-lg-4 d-flex justify-content-center justify-content-md-end align-items-center gap-2">
                                {game.gameIsRunning &&
                                    <a href={'/elimination/' + game.id} className="p-button font-bold">
                                        <GiDart title="Continue Playing" className="fs-6" />
                                    </a>
                                }
                                {!game.gameIsRunning &&
                                    <a href={'/stats/games/elimination/' + game.id} className="p-button font-bold">
                                ...
                                {deleteActive &&
                                    <DeletePopup
                                        ...
                                        handleDelete={onEliminationGameDelete}
                                    />
                                }
                            </div>
                        </div>
            ...
            <div className="container-fluid overflow-hidden">
                <div className="row gy-2">
                    ...
                    <div className="col-12 col-md-4 d-flex align-items-center justify-content-center">
                        <span className="fw-semibold fs-4">Elimination Games</span>
                    </div>
