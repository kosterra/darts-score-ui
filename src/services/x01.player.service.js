...
const { getPlayer, updatePlayer } = PlayerService;

const updateGamePlayerModel = async (game, playerWinner) => {
  let date = new Date();
  ...
  for (const player of game.players) {
    ...
    playerData.players = game.players;
    playerData.playerWinner = playerWinner;
    playerData.gameType = game.gameType;
    playerData.matchLegsInfos = game.allLegsThrows;
  
    playerDataLS.matches.push(playerData);
    playerDataLS.nbrOfMatches++;
    player === playerWinner && game.players.length > 1 && playerDataLS.matchesWon++;
    game.players.length === 1 && playerDataLS.soloGames++;
    ...
    await updatePlayer(player, playerDataLS);
  }
}
...
// Export methods
const X01PlayerService = {
    updateGamePlayerModel
}

export default X01PlayerService;